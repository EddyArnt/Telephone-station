...
> Express REST API with JWT Authentication and support postgresql 

- authentication via [JWT](https://jwt.io/)
- support for [postgresql](https://www.postgresql.org/)
...
Запуск через клонирование репозитария

```sh
# HTTP
$ git clone http://m1trader.cf:708/lk_backend/lk.git
```
...
const hooks = {
  beforeCreate(user) {
    user.password = bcryptSevice.encryptPassword(user);
  },
};
...
Here you define all your routes for your api. It doesn't matter how you structure them. By default they are mapped on `privateRoutes` and `routes`. You can define as much routes files as you want e.g. for every model or for specific use cases, e.g. normal user and admins.
...
```js
const mappedUserRoutes = mapRoutes(config.userRoutes, 'phone-station/controllers/');
...
- `db-migrate create name_branch --config config/config.json` name_branch - название ветки в репозитарии в рамках которой выполняется задача

-  `node_modules/db-migrate/bin/db-migrate create feature_28 --config config/config.json` - если пакет db-migrate не установлен глобально в системе




### Создание теста
- В файле /test/api.test.js 
внутри набора тестов "describe("Тестирование API", function() {"
добавить тест:

    it("POST /private/apimethod", function(done) {
        var values = { 
            "token": token,
            "pool" : {
              "parameter": "parameter"
        ...
        server
            .post('/private/apimethod')
            .send(values)
            .set('Accept', 'application/json')
            .expect(200)
            .end(function(err, res) {
                // список утверждений
                res.body.should.have.property("result");  // тело ответа имеет свойство rusult   
                res.body.result.should.have.property("id", user_id); ("count", ); // тело ответа имеет свойство rusult.id и оно равно переменной user_id
                res.body.should.have.property("success", true); // тело ответа имеет свойство success и оно равно true
                done();
            });
    });
...
или если GET
    ...
    it("GET /private/apimethod", function(done) {
        server
            .get('/private/apimethod')
            .expect(200)
            .end(function(err, res) {
                // список утверждений
...
Изменить список утверждений на нужные.
Утверждения should : https://shouldjs.github.io/

## Swagger

### Запуск
Сервис запускается по адресу /swagger, при старте приложения. Используются следующие npm пакеты: swagger-jsdoc, swagger-ui-express.

### Добавление информации
Документация по swagger [спецификация](https://swagger.io/docs/specification/) [параметры](https://swagger.io/docs/specification/describing-parameters/)
Для описания метода необходимо начать с ключевого слова @swagger, все описание должно быть закоментировано.
...
### Авторизация
Авторизация происходит через всплывающее окно, где необходимо указать ключевое слово Bearer и токен пользователя.
- Формат: "Bearer <token>".
- Пример: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6NSwiaWF0IjoxNTI3MTU3MjI3LCJleHAiOjE1MjcxNjgwMjd9.udDpEXMLLW-AC4H6eSAWPtLFitYRysnT94ZUyqqOJlk

## SOAP

### Авторизация
В тело запроса перед body необходимо вставить header с Bearer токеном.
```js
<soapenv:header
<authorization>eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MiwiaWF0IjoxNTM0MTU4MzI2LCJleHAiOjE1MzQxNjkxMjZ9.4WfTwjSjUbksrmrYb2AYqBJs0GjI2JzFaDaMIcz7WxA</authorization>
</soapenv:header>
```
